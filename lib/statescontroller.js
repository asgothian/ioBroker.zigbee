...
class StatesController extends EventEmitter {
    ...
    leaveDevice(ieeeAddr, model) {
        this.debug(`Leave device event: '${this.deviceLabel(ieeeAddr, model)}'`);
        if (ieeeAddr) {
            ...
            try {
                ...
                states.forEach(statedesc => {
                    ...
                    this.adapter.getState(id, (err, state) => {
                        cnt = cnt + 1;
                        if (!err && state) {
                            this.debug(`collect options for '${this.deviceLabel(devId, model)}':  ${JSON.stringify(result)}`);
                ...
                this.error(`Error collectOptions for '${this.deviceLabel(devId, model)}'. Error: ${error.stack}`);
            }
        }
        catch (error) {
            this.error(`Error in collectOptions for '${this.deviceLabel(devId, model)}' , ${model} : ${error && error.message ? error.message : 'no message given'}`);
            callback({});
    ...
    async getDevStates(deviceId, model) {
        try {
            ...
            this.sendError(error);
            this.error(`Error getDevStates for '${this.deviceLabel(deviceId, model)}'. Error: ${error.stack}`);
        }
    }
    ...
    async publishFromState(deviceId, model, stateKey, state, options, debugID) {
        if (this.debugActive) this.debug(`Change state '${stateKey}' at device '${this.deviceLabel(deviceId, model)}' type '${model}'`);
        const has_elevated_debug = this.checkDebugDevice(typeof deviceId == 'number' ? `group_${deviceId}` : deviceId);

        if (has_elevated_debug)  {
            const message = (`Change state '${stateKey}' at device '${this.deviceLabel(deviceId, model)}' type '${model}'`);
    ...
    async updateDev(dev_id, dev_name, model) {
        ...
        if (this.debugActive) this.debug(`UpdateDev called with '${this.deviceLabel(dev_id, model)}', ${dev_name}, ${model}, ${__dev_name}`);
        const id = '' + dev_id;
        const objId = model=='group' ? typeof dev_id == 'string' && dev_id.includes('group') ? dev_id : `group_${dev_id}` : dev_id;
    ...
    async syncDevStates(dev, model) {
        if (this.debugActive) this.debug('synchronizing device states for ' + "'" + this.deviceLabel(dev.ieeeAddr, model) + "'" + ' (' + model + ')');
    ...
    async publishToDefinedState(devId, model, statedesc, payload, debugId) {
        ...
        if (value === undefined || value === null) {
            const message = `value '${JSON.stringify(value)}' from device '${this.deviceLabel(devId, model)}' via ${mode} for '${statedesc.name} (ID ${statedesc.id} ${statedesc.prop ? 'with property '+statedesc.prop : ''})`;
        ...
        const message = `value generated '${typeof(value) == 'object' ? JSON.stringify(value) : value}' from device '${this.deviceLabel(devId, model)}' for '${statedesc.name}'`;
    ...
    async publishToState(devId, model, payload, debugId) {
        try {
            ...
            const message = `message received '${JSON.stringify(payload)}' from device '${this.deviceLabel(devId, model)}' type '${model}'`;
        ...
        for (const converter of converters) {
            ...
            chain.push(new Promise((resolve) => {
                ...
                try {
                    ...
                    const msg = `Error while processing converters DEVICE_ID: '${this.deviceLabel(devId, model)}' cluster '${converter.cluster}' type '${converter.type}' ${error && error.message ? ' message: ' + error.message : ''}`;
    ...
    async onZigbeeEvent(type, entity, message) {
        ...
        if (device.interviewState === 'IN_PROGRESS') {
            this.debug(`zigbee event for '${this.deviceLabel(device.ieeeAddr, model)}' received during interview!`);
            return;
        }
        ...
        if (type === 'commandQueryNextImageRequest' && device.type !== 'Coordinator') {
            try {
                ...
                this.debug(`Answered unsolicited OTA query from '${this.deviceLabel(device.ieeeAddr, model)}' with NO_IMAGE_AVAILABLE`);
            } catch (error) {
                this.error(`Failed to answer unsolicited OTA query from '${this.deviceLabel(device.ieeeAddr, model)}' (${error && error.message ? error.message : error})`);
            }
        }

        // everything from here on needs the model definition and the objects of the device
        if (!this.isDeviceRegistered(entity)) {
            this.debug(`message from '${this.deviceLabel(device.ieeeAddr, model)}' dropped - the device is not registered yet`);
            return;
        }
        ...
        if (has_elevated_debug) {
            const message = `Zigbee Event of Type ${type} from device '${this.deviceLabel(device.ieeeAddr, model)}', incoming event: ${safeJsonStringify(msgForState)}`;
        ...
        if (has_elevated_debug) {
            const message = `${converters.length} converter${converters.length > 1 ? 's' : ''} available for '${mappedModel.model}' '${this.deviceLabel(devId, model)}' with cluster '${cluster}' and type '${type}'`;
        ...
        if (!converters.length) {
            if (type !== 'readResponse' && type !== 'commandQueryNextImageRequest') {
                const message = `No converter available for '${mappedModel.model}' '${this.deviceLabel(devId, model)}' with cluster '${cluster}' and type '${type}'`;
        ...
        try {
            ...
            if (cluster !== '64529') {
                ...
                this.error(`Error while processing converters DEVICE_ID: '${this.deviceLabel(devId, model)}' cluster '${cluster}' type '${type}'`);
