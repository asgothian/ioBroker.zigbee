...
class StatesController extends EventEmitter {
    ...
    async processConverters(converters, devId, model, mappedModel, message, meta, metaStates, debugId, has_elevated_debug) {
        ...
        if (has_elevated_debug) {
            ...
            {
                if (k == 'device') {
                    metapost.IEEE = meta.device?.ieeeAddr ?? 'no IEEE';
                    metapost.Model = meta.device?.modelID ?? 'no modelID';
                    continue;
                }
                if (k == 'logger') {
                    metapost.logger = meta.logger ? meta.logger.constructor?.name ?? 'not a class' : 'not set';
                    continue;
                }
                metapost[k] = meta[k];
            }
            metamsg = ` data: ${safeJsonStringify(message.data)} options: ${safeJsonStringify(options)} meta:${safeJsonStringify(metapost)}`
        }
        ...
        /* for (const converter of converters) {
            ...
            chain.push(new Promise((resolve) => {
                ...
                try {
                    ...
                    if (has_elevated_debug) {
                        this.emit('device_debug', {ID:debugId,data: { flag:`02.${idx}b`, IO:true }, message:`${metamsg} result:${safeJsonStringify(payloadConv)}`})
                    }
                    if (typeof payloadConv === 'object') {
                        resolve({ result: payloadConv, meta: meta });
                    }
                    else resolve({});
                }
                catch (error) {
        ...
        for (const candidate of candidates) {
            for (const key in candidate.result ?? {}) {
                payload[key] = candidate.result[key];
            }
            for (const key in candidate.meta ?? {}) {
                UpdatedMeta[key] = candidate.meta[key];
            }
        }
        */
        // serial
        if (has_elevated_debug) this.emit('device_debug', {ID:debugId,data: { flag:`02`, IO:true }, message:`testing ${converters.length} converters with ${metamsg}`})
        let deviceMetaMsg = '';
        if (meta?.device?.meta?.ws90) deviceMetaMsg = `${safeJsonStringify(meta.device.meta.ws90)}`;
        const candidates = [];
        for (const converter of converters) {
            const idx = cnt++;
            try {
                if (has_elevated_debug) this.emit('device_debug', {ID:debugId,data: { flag:`02.${cnt}a`, IO:true }, message:`converter ${cnt} : Cluster ${converter.cluster}, device.meta: ${safeJsonStringify(meta?.device?.meta ?? 'no device meta')}`});
                const payloadConv = await converter.convert(mappedModel, message, publish, options, meta);
                if (has_elevated_debug) {
                    this.emit('device_debug', {ID:debugId,data: { flag:`02.${idx}b`, IO:true }, message:`${metamsg} device.meta: ${safeJsonStringify(meta?.device?.meta ?? 'no device meta')} result:${safeJsonStringify(payloadConv)}`})
                }
                if (typeof payloadConv === 'object') {
                    candidates.push(payloadConv);
                }
            }
            catch (error) {
                const msg = `Error while processing converters DEVICE_ID: '${devId}' cluster '${converter.cluster}' type '${converter.type}' ${error && error.message ? ' message: ' + error.message : ''}`;
        ...
        if (meta?.device?.meta?.ws90) {
            const postMetaMsg = `${safeJsonStringify(meta.device.meta.ws90)}`;
            if (postMetaMsg != deviceMetaMsg) {
                this.warn(`meta.device.meta.ws90 changed from ${deviceMetaMsg} to ${postMetaMsg} with payload ${JSON.stringify(payload)}`);
            }
            else
                this.warn(`meta.device.meta.ws90 was unchanged changed from ${postMetaMsg} with payload ${JSON.stringify(payload)}`);
        }

        if (Object.keys(payload).length > 0 && Object.keys(options).length > 0) {
            let premsg = '';
            //if (has_elevated_debug) premsg = `candidates: ${safeJsonStringify(candidates.map((obj) => obj.result))} => payload ${safeJsonStringify(payload)}`
            if (has_elevated_debug) premsg = `candidates: ${safeJsonStringify(candidates)} => payload ${safeJsonStringify(payload)}`
        ...
        if (!metaStates.includes('state')) delete meta.state;
    ...
    async updateMeta(devId, meta) {
        try {
            ...
            if (changed) {
                ...
                //    this.warn(`Statescontroller  no update meta for ${devId} => meta was unchanged (${JSON.stringify(m)})`);
            }
        }
        catch (error) {
