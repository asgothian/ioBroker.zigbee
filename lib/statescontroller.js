...
class StatesController extends EventEmitter {
    ...
    async processConverters(converters, devId, model, mappedModel, message, meta, metaStates, debugId, has_elevated_debug) {
        ...
        if (has_elevated_debug) {
            ...
            {
                if (k == 'device') {
                    metapost.deviceIEEE = meta.device?.ieeeAddr ?? 'no IEEE';
                    meta.deviceModelId = meta.device?.ModelId ?? 'no modelID';
                    continue;
                }
                if (k == 'logger') {
                    metapost.logger = meta.logger ? meta.logger.constructor ?? 'not a class' : 'not set';
                    continue;
                }
                metapost[k] = meta[k];
            }
