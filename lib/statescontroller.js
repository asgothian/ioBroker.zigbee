...
class StatesController extends EventEmitter {
    ...
    error(message, data) {
        this.emit('log', 'error', `StatesController:${message}`, data);
    }

    debug(message, data) {
        this.emit('log', 'debug', `StatesController:${message}`, data);
    }

    warn(message, data) {
        this.emit('log', 'warn', `StatesController:${message}`, data);
    }
    ...
    leaveDevice(ieeeAddr, model) {
        this.debug(`Leave device event: '${this.internalDevLabel(ieeeAddr, model)}'`);
        if (ieeeAddr) {
            ...
            try {
                ...
                states.forEach(statedesc => {
                    ...
                    this.adapter.getState(id, (err, state) => {
                        cnt = cnt + 1;
                        if (!err && state) {
                            this.debug(`collect options for '${this.internalDevLabel(devId, model)}':  ${JSON.stringify(result)}`);
                ...
                this.error(`Error collectOptions for '${this.internalDevLabel(devId, model)}'. Error: ${error.stack}`);
            }
        }
        catch (error) {
            this.error(`Error in collectOptions for '${this.internalDevLabel(devId, model)}' , ${model} : ${error && error.message ? error.message : 'no message given'}`);
            callback({});
    ...
    async getDevStates(deviceId, model) {
        try {
            ...
            this.sendError(error);
            this.error(`Error getDevStates for '${this.internalDevLabel(deviceId, model)}'. Error: ${error.stack}`);
        }
    }
    ...
    async triggerComposite(_deviceId, stateDesc, interactive, stateDefinitions) {
        ...
        this.compositeData[key].handle = this.adapter.setTimeout(async () => {
            for (const trigger of this.compositeData[key].triggers) {
                this.emit('acknowledge_state', deviceId, trigger, undefined );
            }
    ...
    async publishFromState(deviceId, model, stateKey, state, options, debugID) {
        if (this.debugActive) this.debug(`Change state '${stateKey}' at device '${this.internalDevLabel(deviceId, model)}' type '${model}'`);
        const has_elevated_debug = this.checkDebugDevice(typeof deviceId == 'number' ? `group_${deviceId}` : deviceId);

        if (has_elevated_debug)  {
            const message = (`Change state '${stateKey}' at device '${this.internalDevLabel(deviceId, model)}' type '${model}'`);
    ...
    verifyDeviceName(id, model, name) {
        ...
        // ae: removed deviceNameCache, as it makes no sense:
        // - if the device has not been renamed, the name will be the model.
        // - if the device has been renamed, the name will be in the localConfig
        //. toVerify: whats with groups ???
        return this.localConfig.NameForId(id, model, name ?? model);
    }
    ...
    async updateDev(dev_id, dev_name, model) {
        ...
        if (this.debugActive) this.debug(`UpdateDev called with '${this.internalDevLabel(dev_id, model)}', ${dev_name}, ${model}, ${__dev_name}`);
        const id = '' + dev_id;
        ...
        if (obj) {
            await this.adapter.extendObjectAsync(id, {
                type: 'device',
                common: myCommon,
                native: { modelIcon: modelIcon, id:dev_id, model:model }
            });
        } else {
    ...
    async syncDevStates(dev, model) {
        if (this.debugActive) this.debug('synchronizing device states for ' + "'" + this.internalDevLabel(dev.ieeeAddr, model) + "'" + ' (' + model + ')');
    ...
    async publishToDefinedState(devId, model, statedesc, payload, debugId) {
        ...
        if (value === undefined || value === null) {
            const message = `value '${JSON.stringify(value)}' from device '${this.internalDevLabel(devId, model)}' via ${mode} for '${statedesc.name} (ID ${statedesc.id} ${statedesc.prop ? 'with property '+statedesc.prop : ''})`;
        ...
        const message = `value generated '${typeof(value) == 'object' ? JSON.stringify(value) : value}' from device '${this.internalDevLabel(devId, model)}' for '${statedesc.name}'`;
    ...
    async publishToState(devId, model, payload, debugId) {
        try {
            ...
            const message = `message received '${JSON.stringify(payload)}' from device '${this.internalDevLabel(devId, model)}' type '${model}'`;
        ...
        for (const converter of converters) {
            ...
            chain.push(new Promise((resolve) => {
                ...
                try {
                    ...
                    const msg = `Error while processing converters DEVICE_ID: '${this.internalDevLabel(devId, model)}' cluster '${converter.cluster}' type '${converter.type}' ${error && error.message ? ' message: ' + error.message : ''}`;
    ...
    async onZigbeeEvent(type, entity, message) {
        ...
        if (entity.device.interviewing) {
            this.debug(`zigbee event for '${this.internalDevLabel(device.ieeeAddr, model)}' received during interview!`);
            return;
        }
        ...
        if (type === 'commandQueryNextImageRequest' && device.type !== 'Coordinator') {
            try {
                ...
                this.debug(`Answered unsolicited OTA query from '${this.internalDevLabel(device.ieeeAddr, model)}' with NO_IMAGE_AVAILABLE`);
            } catch (error) {
                this.error(`Failed to answer unsolicited OTA query from '${this.internalDevLabel(device.ieeeAddr, model)}' (${error && error.message ? error.message : error})`);
            }
        }
        ...
        if (has_elevated_debug) {
            const message = `Zigbee Event of Type ${type} from device '${this.internalDevLabel(device.ieeeAddr, model)}', incoming event: ${safeJsonStringify(msgForState)}`;
        ...
        if (has_elevated_debug) {
            const message = `${converters.length} converter${converters.length > 1 ? 's' : ''} available for '${mappedModel.model}' '${this.internalDevLabel(devId, model)}' with cluster '${cluster}' and type '${type}'`;
        ...
        if (!converters.length) {
            if (type !== 'readResponse' && type !== 'commandQueryNextImageRequest') {
                const message = `No converter available for '${mappedModel.model}' '${this.internalDevLabel(devId, model)}' with cluster '${cluster}' and type '${type}'`;
        ...
        try {
            ...
            if (cluster !== '64529') {
                ...
                this.error(`Error while processing converters DEVICE_ID: '${this.internalDevLabel(devId, model)}' cluster '${cluster}' type '${type}'`);
