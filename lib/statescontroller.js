...
class StatesController extends EventEmitter {
    ...
    async handleStateResend(entity, end_with_device_query) {
        ...
        if (end_with_device_query) states.device_query = true;
    ...
    async publishFromState(deviceId, model, stateKey, state, options, debugID) {
        ...
        if (stateDesc.id === 'device_query') {
            //this.emit('read_states', deviceId, model, stateModel, devStates.states.map((obj) => obj?.id), options, debugID, has_elevated_debug );
            //return;
    ...
    async syncDevStates(dev, model) {
        ...
        for (const stateInd in states) {
            ...
            if (statedesc === undefined) {
                this.error(`syncDevStates: Illegal state in ${JSON.stringify(utils.deviceData(dev))} -  ${JSON.stringify(stateInd)}`);
                return;
            }
    ...
    async publishToDefinedState(devId, model, statedesc, payload, debugId) {
        ...
        const rawValue = payload[statedesc.id] ?? payload[statedesc.prop];

        if ((rawValue ?? '##NOVAL##') == '##NOVAL##')
            return {} //there's nothing for this
        let value = undefined;
        let mode = 'prop/id'
        if (statedesc.getter) {
        ...
        const message = `value generated '${typeof(value) == 'object' ? JSON.stringify(value) : value}' from device '${this.devLabel(devId, model)}' via ${mode} for '${statedesc.name}'`;
        ...
        const key = (payload[statedesc.id] ?? '###') == '###' ? statedesc.id : statedesc.prop;
    ...
    async publishToState(devId, model, payload, debugId) {
        try {
            ...
            if (devStates.states !== undefined) {
                try {
                    const states = devStates.states.filter(statedesc => payload.hasOwnProperty(statedesc.prop) || payload.hasOwnProperty(statedesc.id));
    ...
    getEpNameFromMappedModel(mm, device, id) {
        if (!mm || id<0) return undefined;
        try {
            const epNameObj = mm.endpoint(device);
            for (const key of Object.keys(epNameObj)) {
                if (epNameObj[key] == id) return key;
            }
        }
        catch {
            // intentionally empty
    ...
    async processConverters(converters, devId, model, mappedModel, message, meta, debugId, has_elevated_debug) {
        let cnt = 0;
        const epname = this.getEpNameFromMappedModel(mappedModel,message?.device, message?.endpoint?.ID ?? -1);
            ...
            chain.push(new Promise((resolve) => {
                ...
                try {
                    ...
                    if (typeof payloadConv === 'object') {
                        const keyArr = Object.keys(payloadConv);
                        if (epname) for (const key of keyArr) {
                            if (key.endsWith(epname))
                                payloadConv[key.replace(`_${epname}`, '')] = payloadConv[key];
                        }
                        resolve(payloadConv);
                    }
                    else resolve({});
