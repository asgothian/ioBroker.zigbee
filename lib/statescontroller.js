...
//            const stateKey = id.split('.')[3];
    ...
    async getExcludeExposes(allExcludesObj) {
        statesMapping.fillStatesWithExposes(allExcludesObj, this.adapter.log.warn);
    }
    ...
    async publishToState(devId, model, payload) {
        ...
        if (devStates.states !== undefined) {
            ...
            this.warn(`ELEVATED publishToState: no state found for payload '${JSON.stringify(payload)}' from device ${devId} type '${model}'`);
        }
    }
}
