...
class DeviceAvailability extends BaseExtension {
    ...
    async registerDevicePing(device, entity) {
        ...
        this.debug(`register device Ping for '${this.internalDevLabel(device.ieeeAddr, entity?.mapped?.model)}'`);
        ...
        this.debug(`registering device Ping (${this.number_of_registered_devices}) for '${this.internalDevLabel(device.ieeeAddr, entity?.mapped?.model)}'`);
    ...
    async deregisterDevicePing(device) {
        this.debug(`deregister device Ping for deactivated device '${this.internalDevLabel(device.ieeeAddr)}'`);
    ...
    async onZigbeeStarted() {
        ...
        for (const device of clients) {
            if (this.isPingable(device) && this.active_ping) {
                ...
                this.debug(`Setting '${this.internalDevLabel(device.ieeeAddr)}'  as available - battery driven or no active availability check`);
                this.publishAvailability(device, true);
                this.timers[device.ieeeAddr] = setInterval(() =>
                    this.handleIntervalNotPingable(device), utils.secondsToMilliseconds(this.availability_timeout));
            }
            ...
            this.zigbee.adapter.setTimeout(() => {
                readables.forEach(device =>
                    this.zigbee.doDeviceQuery(device, Date.now(), false).catch(error =>
                        this.warn(`device query for '${this.internalDevLabel(device.ieeeAddr)}' failed: ${error && error.message ? error.message : error}`)));
            }, this.startReadDelay)
        }
    }
    ...
    async handleIntervalPingable(device, entity) {
        ...
        if (!resolvedEntity) {
            const msg = `Stop pinging '${this.internalDevLabel(ieeeAddr)}' ${device.modelID}, device is not known anymore`
        ...
        if (this.isPingable(device)) {
            ...
            try {
                if (!this.pingCluster || !this.pingCluster.hasOwnProperty('id')) {
                    const message = `Pinging '${this.internalDevLabel(ieeeAddr, resolvedEntity.mapped?.model)}' (${device.modelID}) via ZH Ping`
                    ...
                    const message = `Pinging '${this.internalDevLabel(ieeeAddr, resolvedEntity.mapped?.model)}' (${device.modelID}) via ZCL Read with ${this.pingCluster.id}:${this.pingCluster.attribute}`
                    if (has_elevated_debug) {
                ...
                if (has_elevated_debug) {
                    const message = `Successfully pinged '${this.internalDevLabel(ieeeAddr, resolvedEntity.mapped?.model)}' in ${Date.now()-debugID} ms`
                ...
                if (error && error.message && error.message.includes('UNSUPPORTED_ATTRIBUTE')) {
                    ...
                    this.publishAvailability(device, true);
                    if (has_elevated_debug)
                        this.zigbee.emit('device_debug', {  ID:debugID, data: {ID: device.ieeeAddr, flag:'SUCCESS', IO:false}, message:`Successfully pinged '${this.internalDevLabel(ieeeAddr, resolvedEntity.mapped?.model)}' in ${Date.now()-debugID} ms`});
                    this.setTimerPingable(device, 1);
                    this.ping_counters[device.ieeeAddr].failed = 0;
                    return;
                }
                if (has_elevated_debug)
                    this.zigbee.emit('device_debug',{ ID:debugID, data: {ID: device.ieeeAddr, error:'PIFAIL', IO:false}, message:`Failed to ping '${this.internalDevLabel(ieeeAddr, resolvedEntity.mapped?.model)}' after ${Date.now()-debugID} ms${error && error.message ? ' - '+error.message : ''}`});
                this.publishAvailability(device, false, false, has_elevated_debug ? debugID : undefined);
                if (pingCount.failed++ <= this.max_ping) {
                    const message = `Failed to ping '${this.internalDevLabel(ieeeAddr, resolvedEntity.mapped?.model)}' for ${JSON.stringify(pingCount)} attempts`
                    ...
                    const msg = `Stopping to ping '${this.internalDevLabel(ieeeAddr, resolvedEntity.mapped?.model)}' after ${pingCount.failed} ping attempts`;
                    if (has_elevated_debug)
    ...
    async handleIntervalNotPingable(device) {
        ...
        if (ago > Hours25) {
            ...
            const msg = `Non-pingable device '${this.internalDevLabel(entity.device.ieeeAddr, entity.mapped?.model)}' was last seen over 25 hrs ago - setting it offline.`
            if (this.checkDebugDevice(device.ieeeAddr)) this.warn(`ELEVATED: ${msg}`); else this.debug(msg);
            return;
        }
        const msg = `Non-pingable device '${this.internalDevLabel(entity.device.ieeeAddr, entity.mapped?.model)}' was last seen '${ago / 1000}' seconds ago.`
        if (this.checkDebugDevice(device.ieeeAddr)) this.warn(`ELEVATED: ${msg}`); else this.debug(msg);
    }
    ...
    async onReconnect(device) {
        ...
        if (entity && entity.mapped) {
            ...
            try {
                ...
                this.debug(`Failed to read state of '${this.internalDevLabel(entity.device.ieeeAddr, entity.mapped.model)}' after reconnect`);
            }
        }
    }
    ...
    async publishAvailability(device, available, force) {
        ...
        if (entity && entity.mapped) {
            ...
            if (force || (astate.available !== available) || (now - astate.ts > this.availableTime)) {
                ...
                this.debug(`Publish available for '${this.internalDevLabel(ieeeAddr)}' = ${available}`);
                ...
                if (force || (astate.available !== available)) {
                    this.debug(`Publish LQ for '${this.internalDevLabel(ieeeAddr)}' = ${(available ? 10 : 0)}`);
