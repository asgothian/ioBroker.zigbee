...
class DelayedAction extends BaseExtension {
    ...
    delayAction(device, action) {
        try {
            ...
            this.debug(`Succesfully delay action for '${this.internalDevLabel(device.ieeeAddr)}'`);
            this.doActions(device);
        } catch (error) {
            this.sendError(error);
            this.error(
                `Failed to DelayedAction.delayAction '${this.internalDevLabel(device.ieeeAddr)}' (${error.stack})`,
            );
        }
    }
    ...
    async doActions(device) {
        try {
            ...
            if (!foundDev) {
                this.debug(`No found device '${this.internalDevLabel(device.ieeeAddr)}', for doAction`);
            ...
            for (const actionDef of deviceActions) {
                ...
                this.debug(`Do action on '${this.internalDevLabel(device.ieeeAddr)}'`);
                try {
                    // do action
                    await actionDef.action(device);
                    this.info(`Do action successfully '${this.internalDevLabel(device.ieeeAddr)}'`);
                    toDelete.push(actionDef);
                } catch (error) {
                    this.sendError(error);
                    this.error(
                        `Failed to do action '${this.internalDevLabel(device.ieeeAddr)}', ` +
            ...
            this.sendError(error);
            this.error(
                `Failed to DelayedAction.doAction '${this.internalDevLabel(device.ieeeAddr)}' (${error.stack})`,
            );
        }
    }
}
