...
class Groups extends basePlugin {
    ...
    stop() {
        this.zbController.off('published', this.onGroupStatePublished);
        this.stController.off('changed', this.onGroupStatePublished);
        super.stop();
    ...
    async readMemberStatus(member, item, toRead) {
        ...
        if (obj && obj.common.deactivated) {
            this.debug(`omitting reading member state for deactivated device '${this.internalDevLabel( member.ieee, mappedModel.model)}'`);
            return;
        }
        ...
        if (canReadViaConverter) {
            try {
                ...
                this.debug(`reading ${item.state} from '${this.internalDevLabel(member.ieee, mappedModel.model)}'${member.ieee ? '/' + member.epid : ''} via convertGet failed with ${error && error.message ? error.message : 'no reason given'}`);
                return {unread:member.device};
            }
            this.debug(`reading ${item.state} from '${this.internalDevLabel( member.ieee, mappedModel.model)}'${member.ep ? '/' + member.epid : ''} via convertGet succeeded` );
            ...
            if (device && endpoint) {
                if (entity.endpoint.inputClusters.includes(toRead.cluster)) {
                    try {
                        ...
                        this.debug(`readGroupMemberStatus for ${item.state} from '${this.internalDevLabel( member.ieee, mappedModel.model)}'${member.ep ? '/' + member.epid : ''} resulted in ${JSON.stringify(result)}`);
                    }
                    catch (error) {
                        this.debug(`reading ${item.state} from '${this.internalDevLabel( member.ieee, mappedModel.model)}'${member.ep ? '/' + member.epid : ''} via endpoint.read with ${toRead.cluster}, ${JSON.stringify(toRead.attributes)} resulted in ${error && error.message ? error.message : 'an unspecified error'}`);
                    }
                    this.debug(`reading ${item.state} from '${this.internalDevLabel( member.ieee, mappedModel.model)}'${member.ep ? '/' + member.epid : ''} via endpoint.read with ${toRead.cluster}, ${JSON.stringify(toRead.attributes)} succeeded`);
                }
                else this.debug(`omitting cluster ${toRead.cluster} - not supported`);
            }
            else {
                this.debug(`unable to read ${item.state} from '${this.internalDevLabel( member.ieee, mappedModel.model)}'${member.ep ? '/' + member.epid : ''} - unable to resolve the entity`);
            }
            return;
        }
    }
