...
class Groups extends basePlugin {
    constructor(adapter) {
        super(adapter, 'Group')
    ...
    start(zbController, stController) {
        ...
        this.registerHandler(zbController,'published', this.onGroupStatePublished.bind(this));
        this.registerHandler(stController,'changed', this.onDeviceStateChanged.bind(this));
    ...
    async readMemberStatus(member, item, toRead) {
        ...
        if (obj && obj.common.deactivated) {
            if (this.debugActive) this.debug(`omitting reading member state for deactivated device '${this.deviceLabel(member.ieee, mappedModel.model)}'`);
            return;
        }
        ...
        if (canReadViaConverter) {
            try {
                ...
                if (this.debugActive) this.debug(`reading ${item.state} from '${this.deviceLabel(member.ieee, mappedModel.model)}'${member.ieee ? '/' + member.epid : ''} via convertGet failed with ${error && error.message ? error.message : 'no reason given'}`);
                return {unread:member.device};
            }
            if (this.debugActive) this.debug(`reading ${item.state} from '${this.deviceLabel(member.ieee, mappedModel.model)}'${member.ep ? '/' + member.epid : ''} via convertGet succeeded` );
            ...
            if (device && endpoint) {
                if (entity.endpoint.inputClusters.includes(toRead.cluster)) {
                    try {
                        ...
                        if (this.debugActive) this.debug(`readGroupMemberStatus for ${item.state} from '${this.deviceLabel(member.ieee, mappedModel.model)}'${member.ep ? '/' + member.epid : ''} resulted in ${JSON.stringify(result)}`);
                    }
                    catch (error) {
                        if (this.debugActive) this.debug(`reading ${item.state} from '${this.deviceLabel(member.ieee, mappedModel.model)}'${member.ep ? '/' + member.epid : ''} via endpoint.read with ${toRead.cluster}, ${JSON.stringify(toRead.attributes)} resulted in ${error && error.message ? error.message : 'an unspecified error'}`);
                    }
                    if (this.debugActive) this.debug(`reading ${item.state} from '${this.deviceLabel(member.ieee, mappedModel.model)}'${member.ep ? '/' + member.epid : ''} via endpoint.read with ${toRead.cluster}, ${JSON.stringify(toRead.attributes)} succeeded`);
                }
                else if (this.debugActive) this.debug(`omitting cluster ${toRead.cluster} - not supported`);
            }
            else {
                if (this.debugActive) this.debug(`unable to read ${item.state} from '${this.deviceLabel(member.ieee, mappedModel.model)}'${member.ep ? '/' + member.epid : ''} - unable to resolve the entity`);
            }
            return;
        }
    }
    ...
    async getGroups(obj) {
        if (this.debugActive) this.debug('get groupes called with ' + JSON.stringify(obj));
        const response = {groups: {}};

        const isEnable = await this.adapter.getStateAsync('info.connection');
        if (this.debugActive) this.debug('get groupes called with ' + (obj ? JSON.stringify(obj) : 'no object') + ' ' + (isEnable.val ? 'connected' : 'disconnected'));
        if (isEnable.val) {
            try {
                ...
                if (this.debugActive) this.debug(`getGroups result: ${JSON.stringify(groups)} ( ${JSON.stringify(herdsmanGroups)})`);
    ...
    async updateGroupMembership(from, command, message, callback) {
        try {
            ...
            if (this.debugActive) this.debug('updateGroupMembership called with ' + JSON.stringify(devId));
            if (devId === 'NODEVICE') {
            ...
            for (const epid in groups) {
                for (const gpid of groups[epid]) {
                    ...
                    if (gpidn < 0) {
                        ...
                        if (this.debugActive) this.debug(`calling removeDevFromGroup with ${sysid}, ${-gpidn}, ${epid}` );
                        ...
                        GroupsToSync.push(gpidn);
                        if (this.debugActive) this.debug(`calling addDevToGroup with ${sysid}, ${gpidn}, ${epid}` );
    ...
    async queryGroupMemberState(groupID, stateDesc) {
        ...
        for (const member of members) {
            ...
            if (this.debugActive) this.debug(`entity: ${JSON.stringify(entityData(entity))}`);
            const mappedModel = entity.mapped;
            if (this.debugActive) this.debug(`Mapped Model: ${JSON.stringify(mappedModel)}`);
        ...
        if (result.unread.length > 0) {
            if (this.debugActive) this.debug(`unread ${stateDesc.id} change for group members ${JSON.stringify(result.unread)}`);
        }
    }
    ...
    async renameGroup(from, command, message) {
        if (this.debugActive) this.debug(`rename group called with ${from}, ${command}, ${JSON.stringify(message)}`);
        ...
        try {
            ...
            if (message.remove) {
                for (const member of message.remove) {
                    ...
                    if (this.debugActive) this.debug('trying to remove ' + "'" + this.deviceLabel(member.id) + "'" + (member.ep ? '.'+member.ep : '') + ' ' + ' from group ' + message.id + ' response is '+JSON.stringify(response));
                }
            }
        } catch (e) {
        ...
        if (this.debugActive) this.debug(`rename group name ${message.name}, id ${id}, remove ${JSON.stringify(message.removeMembers)}`);
    ...
    async syncGroups(changed, group_id) {
        ...
        for (const j in groups) {
            ...
            if (this.debugActive) this.debug(`Analysing group_${JSON.stringify(j)}`);
