...
const namedColors = {
    ...
    pink: { rgb:  '#ffc0cb'},
    darkred: { rgb:  '#8b0000'},
    red: { rgb:  '#ff0000', hue: 0, saturation: 100},
...
const CC_DEFS = {
    ...
    COLORS_XY : [['x', 'y']],
    COLORS_XY_RGB : [['hex'],['r', 'g', 'b'],['rgb'],['x', 'y']]
};
...
function parseColorToHSV( RGB ) {
    const rv = {h:0, s:0, v:100}
    const rgb = (typeof RGB == 'string') ? namedColorToRGB(RGB) : RGB;
    if (typeof rgb != 'object') return rv;
    const r = (rgb.r ?? 0) / 255;
    const b = (rgb.b ?? 0) / 255;
    const g = (rgb.g ?? 0) / 255;
    if (r+g+b == 0) return rv;
    const maxc = Math.max(r, g, b)
    const minc = Math.min(r, g, b)
    const v = maxc
    if (minc == maxc) {
        rv.value = Math.floor(v*100)
        return rv;
    }
    rv.s = (maxc-minc) / maxc
    ...
    if (r == maxc) rv.hue = 0.0+bc-gc;
    else if (g == maxc) rv.h = 2.0+rc-bc
    else rv.h = 4.0+gc-rc;
    const D = rv.h/ 6.0;
    rv.h = Math.floor((D - Math.floor(D)) * 360);
    rv.s = Math.floor(rv.s * 100);
    rv.v = Math.floor(rv.v * 100);
    return rv;
}

function capabilities(xyrgb, hs) {
    if (xyrgb && hs) return CC_DEFS.COLORS_ALL;
    if (hs) return CC_DEFS.COLORS_HS;
    if (xyrgb) return CC_DEFS.COLORS_XY_RGB;
    return [];
}
