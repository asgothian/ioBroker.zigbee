...
class Binding extends basePlugin {
    constructor(adapter) {
    ...
    */
    ...
    async doBindUnbind(type, s_address, s_ep, s_clusters, b_address, b_ep, allowAllClusters, callback) {
        try {
            ...
            if (this.debugActive) this.debug(`source: ${safeJsonStringify(source)}`);
            ...
            if (!target) {
                if (b_address === 'coordinator') {
                    ...
                    if (this.debugActive) this.debug(`Coordinator target: ${safeJsonStringify(target)}`);
                } else {
                    target = await this.zbController.resolveEntity(parseInt(b_address));
                    if (this.debugActive) this.debug(`Group target: ${safeJsonStringify(target)}`);
                }
            }
            ...
            if (!found && type != 'unbind') {
                if (this.debugActive) this.debug(`No bind clusters`);
                ...
                for (const clID of clusters) {
                    ...
                    if (targetValid) {
                        if (this.debugActive) this.debug(`${type}ing cluster '${cluster}' from '${sourceName}' to '${targetName}'`);
                        try {
    ...
    async delBinding(from, command, params, callback) {
        try {
            if (this.debugActive) this.debug(`delBinding message: ${JSON.stringify(params)}`);
