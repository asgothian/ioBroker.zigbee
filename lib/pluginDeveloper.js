...
class Developer extends basePlugin {
    constructor(adapter) {
        super(adapter, 'Developer');
    }

    start(zbController, stController) {
        super.start(zbController, stController);
        this.registerHandler(this.adapter,'message', this.onMessage.bind(this));
        this.registerHandler(this.adapter,'sendToZigbee', this.sendToZigbee.bind(this));
    }
    ...
    async sendToZigbee(obj) {
        if (this.zbController) try {
            ...
            try {
                ...
                if (!publishTarget) {
                    if (obj.callback) this.adapter.sendTo(obj.from, obj.command, {localErr: `Device or group '${this.deviceLabel(devId)}' not found!`}, obj.callback);
                    return;
                }
            } catch (error) {
                this.error(`SendToZigbee failed from publishTarget '${this.deviceLabel(devId)}' (${error && error.message ? error.message : 'no details given'})`);
            }
