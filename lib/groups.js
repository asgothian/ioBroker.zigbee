...
class Groups {
    ...
    async readMemberStatus(member, item, toRead) {
        ...
        if (canReadViaConverter) {
            try {
                await converter.convertGet(endpoint, item.state, {device:entity.device, publish: function(data) {}});
            } catch (error) {
    ...
    async queryGroupMemberState(groupID, stateDesc) {
        ...
        for (const member of members) {
            ...
            if (converter.hasOwnProperty('convertGet')) {
                try {
                    const e = entity;
                    await converter.convertGet(entity.device.endpoints[0], stateDesc.id, {device:entity.device, publish:function(data) {
                                                this.warn(`publish called with ${JSON.stringify(data)} on ${entityData(e)}`);
                    }});
                } catch (error) {
