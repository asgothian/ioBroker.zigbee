...
class DeviceEvent extends BaseExtension {
    ...
    async deviceExposeChanged(device, mapped) {
        this.warn(`deviceExposesChanged called with '${this.internalDevLabel(device.ieeeAddr, mapped.model)}' / ${JSON.stringify(mapped.model)}`);
    }
    ...
    async callOnEvent(device, type, data, mappedDevice) {
        ...
        this.debug(`callOnEvent ${type} for ${device?.ieeeAddr}`);
