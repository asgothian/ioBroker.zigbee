...
const { getZbId, reverseByteString, parseIeeeList } = require('./utils.js');
const fs = require('fs');
...
class Commands extends basePlugin {
    ...
    async joinBlocklist(from, command, msg, callback) {
        ...
        const localConfig = this.adapter.localConfig ?? this.stController?.localConfig;
        if (!localConfig) {
    ...
    async deleteZigbeeDevice(from, command, msg, callback) {
        if (this.zbController && this.zbController.herdsmanStarted && this.stController) {
            ...
            if (!dev) {
                ...
                this.info(`Attempted to delete device '${this.deviceLabel(devId)}' - the device is not known to the zigbee controller.`);
            ...
            this.info(`${force ? 'Force removing' : 'Gracefully removing '} device '${this.deviceLabel(devId)}' from the network.`);
