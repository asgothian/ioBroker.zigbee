...
class Commands extends basePlugin {
    ...
    async onMessage(obj) {
        if (typeof obj === 'object' && obj.command) {
            if (obj) {
                switch (obj.command) {
                    case 'testConnect':
                        this.sendTo(obj.from, obj.command, await this.adapter.testConnect(obj.message), obj.callback);
                        break;
                    case 'deleteNVBackup':
                    ...
                    case 'getCoordinatorInfo':
                        if (obj.message && typeof obj.message === 'object') {
                            this.sendTo(obj.from, obj.command, await this.adapter.getCoordinatorInfo(), obj.callback);
                        }
                        break;
                    ...
                    case 'getNamedColors':
                        if (obj.message && typeof obj.message === 'object') {
                            ...
                            this.sendTo(obj.from, obj.command, {colors: val}, obj.callback);
                        }
                        break;
                    ...
                    case 'downloadIcons':
                        ...
                        break;
                    case 'aliveCheck':
                        this.sendTo(obj.from, obj.command, {msg:'success'}, obj.callback);
                        break;
                    case 'clearErrors':
                        this.sendTo(obj.from, obj.command, this.stController.clearStashedErrors(), obj.callback);
                        break;
                    default:
                        //this.debug(`Command ${obj.command} is unknown`);
    ...
    async readNvBackup(from, command, msg, callback) {
        ...
        try {
            ...
            //this.error(msg);
            this.sendTo(from, command, {error:msg}, callback)
        }
    }
    ...
    async delNvBackup(from, command, msg, callback) {
        try {
            ...
            this.sendTo(from, command, {error: error?.message || String(error)}, callback)
            this.error(error);
        }
    }
    ...
    async letsPairing(from, command, message, callback) {
        if (this.zbController && this.zbController.herdsmanStarted) {
            ...
            if (await this.zbController.permitJoin(cTimer, devId)) {
                ...
                //this.sendTo(from, command, cTimer ? 'Start pairing!':'Stop pairing!', callback);
            }
            else {
                this.sendTo(
                    from, command,
    ...
    touchlinkReset(from, command, message, callback) {
        if (this.zbController && this.zbController.herdsmanStarted) {
            ...
            this.sendTo(from, command, 'Start touchlink reset and pairing!', callback);
        } else {
            this.sendTo(
                from, command,
    ...
    renameDevice(from, command, msg, callback) {
        if (this.stController) {
            ...
            this.sendTo(from, command, {}, callback);
        }
    }
    ...
    async deleteZigbeeDevice(from, command, msg, callback) {
        if (this.zbController && this.zbController.herdsmanStarted && this.stController) {
            ...
            if (!dev) {
                ...
                if (!objDelete.status) this.sendTo(from, command, {error: objDelete.message}, callback);
                else this.sendTo(from, command, {}, callback);
                return;
            }
            ...
            this.sendTo(from, command, result, callback);
        } else {
    ...
    async cleanDeviceStates(from, command, force, callback) {
        ...
        const devicesFromDB = await this.zbController.getClientIterator();
        ...
        this.sendTo(from, command, {stateList: messages}, callback);
    }
    ...
    async getLocalImages(from, command, msg, callback) {
        if (this.stController) {
            ...
            this.sendTo(from, command, {imageData:result}, callback)
        }
    }
    ...
    async updateConfigItems(from, command, msg, callback) {
        if (this.stController) {
            ...
            try {
                ...
                this.sendTo(from, command, {err: error.message}, callback);
            }
        }
    }
    ...
    async getLocalConfigItems(from, command, msg, callback)
    {
        ...
        if (this.stController) {
            ...
            if (msg.hasOwnProperty('global') && msg.hasOwnProperty('target') && (msg.hasOwnProperty('keys') || msg.hasOwnProperty('key')))
            {
                ...
                if (msg.getAllData) {
                    this.sendTo(from, command, this.stController.localConfig.localData, callback);
                }
    ...
    async triggerIconDownload(obj) {
        if (!this.stController) {
            this.sendTo(obj.from, obj.command, {msg:'No States controller'}, obj.callback);
            return;
        }
        ...
        if (NumDownloads) {
            this.sendTo(obj.from, obj.command, {msg:`${NumDownloads} downloads triggered.`}, obj.callback);
            Promise.all(Promises);
        }
        else {
            this.sendTo(obj.from, obj.command, {msg:'Nothing to download'}, obj.callback);
        }
