...
function genState(expose, overrides) {
    ...
    const stateName = expose.endpoint ? `${(desc || expose.description || expose.name)} ${String(expose.endpoint).toUpperCase()}` : (desc || expose.description || expose.name);
    ...
    switch (expose.type) {
        case 'binary':
            ...
            if (writable) {
                ...
                if (expose.ep) state.setattr = expose.name;
            }
            if (expose.endpoint) {
    ...
    state.condition = state.condition ?? expose.condition;
    return state;
}
...
function createFromExposes(model, def, device, options, log) {
    ...
    function genStateFromExpose(expose, deviceDefinition) {
        ...
        try {
                            ...
                            case 'brightness': {
                                ...
                                if (definitionHasTZHandler(deviceDefinition, 'brightness_move')) {
                                    ...
                                    pushToStates(modifyState(getStateDefinition('brightness_move'), {
                                        ...
                                        epname: expose.endpoint,
                                        setattr: setattr,
                                        category: prop.category ?? 'dimming',
                                    }, true), prop.access);
                                }
                                break;
                            }
            ...
            if (__logger?.warn) __logger.warn(`error ${_e?.message ?? 'without message'} pushing state generation for device ${device.ieeeAddr} (${model})`);
        }
    }
