...
function createFromExposes(model, def, device, options, log) {
    ...
    function genStateFromExpose(expose, deviceDefinition) {
        ...
        switch (expose.type) {
            ...
            case 'climate':
                if (expose?.features?.[Symbol.iterator]) for (const prop of expose.features) {
                    if (prop.type === 'composite') {
                        if (__logger?.warn) __logger.warn(`pushing composite state generation for feature ${prop.property}`);
                        ...
                        if (__logger?.warn) __logger.warn(`pushing state generation for feature ${prop.property}`);
                        pushToStates(genState(prop), prop.access);
                    }
                }
                break;
            ...
            case 'composite':
            {
                if (__logger?.warn) __logger.warn(`pushing composite state generation for feature ${expose.property}`);
            ...
            default:
                ...
                if (__logger?.warn) __logger.warn(`Unhandled expose type ${expose.type} for device ${model}`)
