...
function genState(expose, p_overrides) {
    const overrides = p_overrides ?? {};
    const role = overrides?.role;
    const name = overrides?.name ?? expose.name;
    const desc = overrides?.desc;
    let state;
    const readable = (expose.access & (ea.STATE_GET)) > 0;
    const writable = (expose.access & ea.SET) > 0;
    const stname = overrides?.property ?? expose.property ?? overrides?.name ?? expose.name;
    if (typeof stname !== 'string') return;
    const stateId = stname.replace(/\*/g, '');
    const stateName = (desc || expose.description || expose.name);
    const property = overrides?.name ?? expose.name;
    const is_inComplex = typeof(stateId) == 'string' ? stateId.includes('.') : false;
    ...
    switch (expose.type) {
        case 'binary':
            ...
            if (readable) {
                state.getter = payload => payload[stateId] === (typeof expose.value_on == 'boolean' ? expose.value_on : expose.value_on || 'ON');
            } else {
        ...
        case 'enum':
            ...
            for (const val of expose.values) {
                ...
                if (val == '') {
                    state.states[property] = property;
                } else {
    ...
    if (overrides?.endpoint ?? expose?.endpoint) {
        ...
        if (overrides?.channelID) {
            ...
            if (expose.access & ea.STATE) {
                expose.getter = payload => {
                    if ((payload.hasOwnProperty(stateId)) && (payload[stateId] !== null) && payload[stateId].hasOwnProperty(stateId)) {
                        return !isNaN(payload[stateId][stateId]) ? payload[stateId][stateId] : undefined;
                    } else {
                        return undefined;
                    }
                };
...
function generateCompositeStates(expose, _channelID, options)
{
    ...
    if (expose?.features?.[Symbol.iterator]) for (const prop of expose.features) {
        if (prop.type === 'composite' || prop.type === 'list') {
            ...
            stateData.states.push(genState(prop, options.newCompositeMethod ? { fromComposite: true, property: `${channelID}${prop.property}` , channelID: stateId }: { fromComposite: true }))
            stateData.keys.push(prop.property)
        }
    }
    if (options.newCompositeMethod) {
