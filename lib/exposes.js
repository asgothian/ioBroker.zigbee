...
const statesDefs = statesLib.states;
...
function createFromExposes(model, def) {
    ...
    for (const expose of def.exposes) {
        let state;
        if (!types.includes(expose.type)) types.push(expose.type);
...
*/
    ...
    const newDev = {
        ...
        states: states,
        exposed: true,
        linkedStates: (types.includes('light') ? [comb.brightnessAndState] : undefined),
...
function applyExposes(mappedDevices, byModel, allExcludesObj, message) {
    // for exlude search
    if (typeof(message) === 'function') {
        message('applyExposes comb ' + JSON.stringify(comb));
        message('applyExposes sync ' + JSON.stringify(sync));
        message('applyExposes devices ' + JSON.stringify(statesLib));
    }
...
//            if (typeof(message) === 'function') message('applyExposes: ' + JSON.stringify(newDevice));
