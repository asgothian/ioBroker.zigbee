...
class Ota extends basePlugin{
    ...
    start(zbController, stController) {
        ...
        this.registerHandler(this.adapter, 'stateChange', this.onStateChange.bind(this));
        ...
        const dataFolder = this.adapter.getDataFolder()
        try {
            setOtaConfiguration(dataFolder);
            this.info(`setting OTA firmware folder to ${dataFolder}.`);
        }
        catch (error) {
            this.error(`Unable to set the Folder for OTA files to ${dataFolder} : ${error?.message ?? ' no message given'}`);
        }
    }
    ...
    async startOtaForEntity(entity) {
        ...
        if (this.inProgress.has(devIeee)) {
            this.error(`Update or check already in progress for '${this.deviceLabel(devIeee, entity?.mapped?.model)}', skipping...`);
            return;
        }
        ...
        if (stateObj && stateObj.common && stateObj.common.deactivated) {
            this.info(`Device '${this.deviceLabel(devIeee, entity?.mapped?.model)}' is deactivated, skipping ota check`);
            return;
        }
        ...
        if ((availablestate && (!availablestate.val)) || (lqi && lqi.val < 1)) {
            this.info(`Device '${this.deviceLabel(devIeee, entity?.mapped?.model)}' is marked unavailable, skipping ota check`);
            return;
        }
        const result = { status: 'unknown', decice:entity?.name ?? 'no name'};
        this.inProgress.add(entity.device.ieeeAddr);
        try {
            this.info('Start firmware update for ' + "'" + this.deviceLabel(entity.device.ieeeAddr, entity.mapped?.model) + "'");

            const onProgress = (progress, remaining) => {
                let message = `Update of '${this.deviceLabel(entity.device.ieeeAddr, entity.mapped?.model)}' at ${progress}%`;
                if (remaining) {
            ...
            const message = `Update of '${this.deviceLabel(entity.device.ieeeAddr, entity.mapped?.model)}' failed (${error.message})`;
            result.status = 'fail';
            result.msg = message;
            this.error(message);
        }
    ...
    async checkDeviceObjects(ieeeAddr) {
        ...
        if ((obj ?? true) && obj?.common?.deactivated) {
            this.info(`Device '${this.deviceLabel(ieeeAddr)}' is deactivated, skipping ota check`);
            return 'deactivated';
        }
        const availablestate = await this.adapter.getStateAsync(`${objID}.available`);
        const lqi = await this.adapter.getStateAsync(`${objID}.link_quality`);
        if ((availablestate && (!availablestate.val)) || (lqi && lqi.val < 1)) {
            this.info(`Device '${this.deviceLabel(ieeeAddr)}' is marked unavailable, skipping ota check`);
            return 'unavailable';
        };
        return '';
    }
    ...
    async checkOtaforEntity(entity) {
        ...
        try {
            this.debug(`Checking if firmware update is available for '${this.deviceLabel(ieeeAddr, entity?.mapped?.model)}'`);
            ...
            if (entity?.mapped?.ota) {
                ...
                if (available.currentFileVersion !== available.otaFileVersion) {
                    this.debug(`current Firmware for '${this.deviceLabel(ieeeAddr, entity?.mapped?.model)}' is ${available.currentFileVersion} new is ${available.otaFileVersion}`);
                }
            } else {
                result.status = 'not_supported';
            }
            this.debug(`Firmware update for '${this.deviceLabel(ieeeAddr, entity?.mapped?.model)}' is ${result.status}`);
        } catch (error) {
            const message = `Failed to check if update available for '${this.deviceLabel(ieeeAddr, entity?.mapped?.model)}' ${error.message}`;
            result.status = 'fail';
            result.msg = message;
    ...
    async startOta(obj) {
        ...
        if (device) {
            ...
            this.debug(`Device '${this.deviceLabel(obj.message.devId)}' is unavailable`);
        }
        this.adapter.sendTo(obj.from, obj.command, result, obj.callback)
    }
