...
class Ota extends basePlugin {
    ...
    async otafromState(devId, statedesc, command) {
        ...
        if (entity) {
            ...
            this.stController.emit('acknowledge_state', devId, statedesc.id, command);
        }
    }
    ...
    async startOtaForEntity(entity) {
        ...
        if (this.inProgress.has(devIeee)) {
            this.error(`Update or check already in progress for '${this.internalDevLabel( devIeee, entity?.mapped?.model)}', skipping...`);
            return;
        }
        ...
        if (stateObj && stateObj.common && stateObj.common.deactivated) {
            this.info(`Device '${this.internalDevLabel( devIeee, entity?.mapped?.model)}' is deactivated, skipping ota check`);
            return;
        }
        ...
        if ((availablestate && (!availablestate.val)) || (lqi && lqi.val < 1)) {
            this.info(`Device '${this.internalDevLabel( devIeee, entity?.mapped?.model)}' is marked unavailable, skipping ota check`);
            return;
        }
        const result = { status: 'unknown', decice:entity?.name ?? 'no name'};
        this.inProgress.add(entity.device.ieeeAddr);
        try {
            this.info(`Start firmware update for '${this.internalDevLabel( entity.device.ieeeAddr, entity.mapped?.model)}'`);

            const onProgress = (progress, remaining) => {
                let message = `Update of '${this.internalDevLabel( entity.device.ieeeAddr, entity.mapped?.model)}' at ${progress}%`;
                if (remaining) {
            ...
            const message = `Update of '${this.internalDevLabel( entity.device.ieeeAddr, entity.mapped?.model)}' failed (${error.message})`;
            result.status = 'fail';
            result.msg = message;
            this.error(message);
        }
    ...
    async checkDeviceObjects(ieeeAddr) {
        ...
        if ((obj ?? true) && obj?.common?.deactivated) {
            this.info(`Device '${this.internalDevLabel( ieeeAddr, obj?.native?.model)}' is deactivated, skipping ota check`);
            return 'deactivated';
        }
        const availablestate = await this.adapter.getStateAsync(`${objID}.available`);
        const lqi = await this.adapter.getStateAsync(`${objID}.link_quality`);
        if ((availablestate && (!availablestate.val)) || (lqi && lqi.val < 1)) {
            this.info(`Device '${this.internalDevLabel( ieeeAddr, obj?.native?.model)}' is marked unavailable, skipping ota check`);
            return 'unavailable';
        };
        return '';
    }
    ...
    async checkOtaforEntity(entity) {
        ...
        try {
            this.debug(`Checking if firmware update is available for '${this.internalDevLabel( ieeeAddr, entity?.mapped?.model)}'`);

            if (entity?.mapped?.ota && entity?.device) {
                ...
                if (available.currentFileVersion !== available.otaFileVersion) {
                    this.debug(`current Firmware for '${this.internalDevLabel( ieeeAddr, entity?.mapped?.model)}' is ${available.currentFileVersion} new is ${available.otaFileVersion}`);
                }
            } else {
                result.status = 'not_supported';
            }
            this.debug(`Firmware update for '${this.internalDevLabel( ieeeAddr, entity?.mapped?.model)}' is ${result.status}`);
        } catch (error) {
            const message = `Failed to check if update available for '${this.internalDevLabel( ieeeAddr, entity?.mapped?.model)}' ${error.message}`;
            result.status = 'fail';
            result.msg = message;
