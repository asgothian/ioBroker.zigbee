...
class ZigbeeController extends EventEmitter {
    ...
    */
    ...
    async start() {
        ...
        try {
            ...
            for (const device of deviceIterator) {
                ...
                const msg = ("'" + this.internalDevLabel(entity.device.ieeeAddr, entity.mapped?.model) + "'" +
    ...
    error(message, data) {
        this.emit('log', 'error', `ZigbeeController:${message}`, data);
    }

    debug(message, data) {
        this.emit('log', 'debug', `ZigbeeController:${message}`, data);
    }

    warn(message, data) {
        this.emit('log', 'warn', `ZigbeeController:${message}`, data);
    }
    ...
    getClientIterator(forbiddenTypes, desiredTypes) {
        const forbiddenArr = forbiddenTypes ?? ['Coordinator'];
        if (Array.isArray(forbiddenArr) && (this.herdsman.database)) {
            return this.herdsman.getDevicesIterator( (device) =>  (Array.isArray(desiredTypes) ? desiredTypes.includes(device?.type) : true) && forbiddenArr.indexOf(device?.type) == -1);
        } else {
            return [].values();
        }
    }
        ...
        try {
            ...
            if (actionResult?.error) {
                ...
                if (!(actionResult?.ignored)) {
                    if (statedesc) {
                        this.emit('acknowledge_state', devId, statedesc.id, command);
                    }
                }
                return true;
            ...
            this.error(`the command ${command} failed for '${this.internalDevLabel(devId)}': ${error?.message}`);
        }
        return false;
    }
    ...
    async reconfigure(devId) {
        ...
        this.debug(`Reconfigure '${this.internalDevLabel(devId)}'`);
    ...
    async interview(devId)
    {
        const entity = await this.resolveEntity(devId);
        if (entity) try {
            this.info(`manual Interview for '${this.internalDevLabel(entity.device.ieeeAddr, entity.mapped?.model)}'`);
            await entity.device.interview(true);
            this.info(`interview for '${this.internalDevLabel(entity.device.ieeeAddr, entity.mapped?.model)}' complete`);
        } catch (error) {
    ...
    async remove(deviceID, force) {
        try {
            ...
            const msg = `Failed to remove ${deviceID ? 'device ' + "'" + this.internalDevLabel(deviceID) + "'" : 'unspecified device'}: ${message}`;
        ...
        if (this.localConfig.optionExists('ignore_device_delete')) {
            ...
            if (entity) {
                if (this.localConfig.getDeviceOption(entity.device.ieeeAddr, entity.mapped?.model, 'ignore_device_delete', true)) {
                    this.warn(`Ignoring device_leave message from '${this.internalDevLabel(message.ieeeAddr, entity.mapped?.model)}' (${entity.device.modelID})`);
                    return;
                }
            }
        }
    ...
    async handleDeviceInterview(message) {
        ...
        */
        try {
            const entity = await this.resolveEntity(message.device || message.ieeeAddr);
            const friendlyName = `${entity.mapped?.model ?? entity.device?.modelID ?? 'unknown'} (${entity.device?.ieeeAddr ?? message.ieeeAddr ?? '0x0000000000000000'})`;
        ...
        try {
            ...
            for (const device of devIter)
            {
                ...
                if (entity.name === 'unresolved device' && this.debugActive) {
                    this.debug('resolve Entity failed for ' + "'" + this.internalDevLabel(device.ieeeAddr) + "'");
                }
    ...
    processSyncStatesList(deviceId, syncStateList) {
        syncStateList.forEach((syncState) => {
            this.emit('acknowledge_state',deviceId, syncState.stateDesc.id, syncState.value);
        });
    }
    ...
    async publish(deviceID, cid, cmd, zclData, cfg, ep, type, callback, zclSeqNum) {
        ...
        if (this.debugActive) this.debug(`Zigbee publish to '${this.internalDevLabel(deviceID, entity.mapped?.model)}', ${cid} - cmd ${cmd} - payload ${JSON.stringify(zclData)} - cfg ${JSON.stringify(cfg)} - endpoint ${ep}`);
    ...
    async publishFromState(deviceId, model, stateModel, stateList, options, debugID, has_elevated_debug) {
        ...
        const deviceLabel = this.internalDevLabel(deviceId, model)

        if (has_elevated_debug)
        {
            const stateNames = stateList.map((state) => state.stateDesc.id);
            const message = `Publishing to '${deviceLabel}' of model ${model} with ${stateNames.join(', ')}`;
            this.emit('device_debug', { ID:debugID, data: { ID: deviceId, flag: '03', IO:false }, message: message});
        }
        else
            if (this.debugActive) this.debug(`main publishFromState : '${deviceLabel}' ${model} ${safeJsonStringify(stateList)}`);
        if (model === 'group') {
            isGroup = true;
        ...
        try {
            ...
            if (this.debugActive) this.debug(`entity: '${deviceLabel}' ${model} ${safeJsonStringify(utils.entityData(entity,{entity:true, device:true, model: true}))}))}`);
            const mappedModel = entity ? entity.mapped : undefined;

            if (!mappedModel) {
                if (this.debugActive) this.debug(`No mapped model for ${model}`);
                if (has_elevated_debug) {
                    const message=`No mapped model '${deviceLabel}' (model ${model})`;
            ...
            for (const changedState of stateList) {
                ...
                if (stateDesc.isOption) {
                    if (has_elevated_debug) {
                        const message = `No converter needed on option state for '${deviceLabel}' of type ${model}`;
                        this.emit('device_debug', { ID:debugID, data: { flag: `SUCCESS` , IO:false }, message:message});
                    }
                    else
                        if (this.debugActive) this.debug(`No converter needed on option state for '${deviceLabel}' of type ${model}`);
                ...
                for (const c of mappedModel.toZigbee) {
                    ...
                    if (!c.hasOwnProperty('key')) {
                        if (converter === undefined)
                        {
                            ...
                            converter = c;
                            if (has_elevated_debug) {
                                const message = `Setting converter to keyless converter for '${deviceLabel}' of type ${model}`;
                                this.emit('device_debug', { ID:debugID, data: { flag: `s4.${msg_counter}` , IO:false }, message:message});
                            }
                            else
                                if (this.debugActive) this.debug(`Setting converter to keyless converter for '${deviceLabel}' of type ${model}`);
                            msg_counter++;
                        }
                        else
                        {
                            if (has_elevated_debug)
                            {
                                const message = `ignoring keyless converter for '${deviceLabel}' of type ${model}`;
                                this.emit('device_debug', { ID:debugID, data: { flag: `i4.${msg_counter}` , IO:false} , message:message});
                            }
                            else
                                if (this.debugActive) this.debug(`ignoring keyless converter for '${deviceLabel}' of type ${model}`);
                            msg_counter++;
                        }
                        continue;
                    }
                ...
                if (converter === undefined) {
                    if (stateDesc.isInternalState) {
                        this.emit('acknowledge_state', deviceId, stateDesc.id, undefined );
                        return;
                    }
                ...
                const message = `convert ${key} with value ${safeJsonStringify(preparedValue)} and options ${safeJsonStringify(preparedOptions)} for device '${deviceLabel}' ${stateDesc.epname ? `with Endpoint ${epName}` : 'without Endpoint'}`;
                if (has_elevated_debug) {
                ...
                do
                {
                    try {
                        ...
                        */
                        ...
                        const message = `convert result ${safeJsonStringify(result)} for device '${deviceLabel}'`;
                        if (isGroup)
                        ...
                        if (result !== undefined) {
                            if (!stateDesc.noack) {
                                this.emit('acknowledge_state', deviceId, stateDesc.id, value );
                            }
                            // process sync state list
                            this.processSyncStatesList(deviceId, syncStateList);
                        }
                        else {
                            if (has_elevated_debug) {
                                ...
                                const message = `Convert does not return a result result for ${key} with ${stringvalue} on device '${deviceLabel}'.`;
                                ...
                                if (readKey.converter) {
                                    ...
                                    setTimeout(async () => {
                                        try {
                                            ...
                                            this.warn(`error reading ${key} from '${deviceLabel}' : ${error?.message}`);
                                        }
                                    }, tt ? tt * 1000+250 : 1000);
                                }
                            }
                        ...
                        if (error.code === 25) {
                            if (retry > 0) {
                                this.warn(`Error ${error.code} on send command to '${deviceLabel}'. (${retry} tries left.), Error: ${error.message}`);
                                retry--;
                                ...
                                this.adapter.filterError(`Error ${error.code} on send command to '${deviceLabel}'.` +
                                ` Error: ${error.message}`, `Send command to ${deviceId} failed with`, error, `send:${String(deviceId).replace('0x', '')}`);
                                if (has_elevated_debug) {
                                    const message = `caught error ${error?.message ?? 'no reason given'} when setting value for device '${deviceLabel}'.`;
                            ...
                            ` Error: ${error.message}`, `Send command to ${deviceId} failed with`, error, `send:${String(deviceId).replace('0x', '')}`);
                            if (has_elevated_debug) {
                                const message = `caught error ${error?.message ?? 'no reason given'} when setting value for device '${deviceLabel}' (${error?.stack ?? ''}).`;
                                this.emit('device_debug', { ID:debugID, data: { error: `EXSET${error.code == 25 ? retry : ''}` , IO:false },message:message});
                            }
                        }
                    }
            ...
            const message = `No entity for '${deviceLabel}' : ${err && err.message ? err.message : 'no error message'}`;
            if (has_elevated_debug) {
            ...
            try {
                ...
                try {
                    ...
                    if (payloadObj.acknowledge) {
                        this.emit('acknowledge_state', payloadObj.device, 'send_payload', undefined);
                    }
                    return {success: true};
                } catch (error) {
                    // payloadObj, not payload: payload may be the raw JSON string (no .device)
                    this.error(`Error ${error.code} on send command to '${this.internalDevLabel(payloadObj.device, mappedModel.model)}'.` + ` Error: ${error.stack} ` + `Send command to '${this.internalDevLabel(payloadObj.device, mappedModel.model)}' failed with ` + error);
                    this.adapter.filterError(`Error ${error.code} on send command to '${this.internalDevLabel(payloadObj.device, mappedModel.model)}'.` + ` Error: ${error.stack}`, `Send command to '${this.internalDevLabel(payloadObj.device, mappedModel.model)}' failed with`, error);
    ...
    async doDeviceQuery(deviceId, debugID, elevated) {
        ...
        if (elevated) {
            const message  = `Device query for '${this.internalDevLabel(entity.device.ieeeAddr, mappedModel.model)}' triggered`;
            ...
            this.info(`Device query for '${this.internalDevLabel(entity.device.ieeeAddr, mappedModel.model)}' started`);
        }
        const meta = await utils.getMeta(this.adapter, `${utils.zbIdorIeeetoAdId(this.adapter, deviceId, false)}`);
        ...
        for (const target of targets) {
            ...
            try {
                ...
                read++;
                if (elevated) {
                    const message = `reading ${target.key} from Device ${this.internalDevLabel(entity.device.ieeeAddr, mappedModel.model)}} ${target.epName ? 'Endpoint '+target.epName : ''} in ${(Date.now()-start)} ms`;
                    this.emit('device_debug', { ID:debugID, data: { flag: `qs${attempt}` , IO:false }, message:message});
                }
            }
            catch (e) {
                if (elevated) {
                    const message = `failed to read ${target.key} from Device ${this.internalDevLabel(entity.device.ieeeAddr, mappedModel.model)}} ${target.epName ? 'Endpoint '+target.epName : ''} after ${(Date.now()-start)} ms`;
        ...
        if (elevated) {
            const message = `Device query for '${this.internalDevLabel(entity.device.ieeeAddr, mappedModel.model)}}' complete`;
            ...
            this.info(`Device query for '${this.internalDevLabel(entity.device.ieeeAddr, mappedModel.model)}' complete, read ${read} of ${targets.length} possible states`);

    }

    async deviceQuery(deviceId, debugID, elevated, callback) {
        if (this.deviceQueryActive.includes (deviceId)) {
            this.info(`Device query for '${this.internalDevLabel(deviceId)}' is still active.`);
            return;
        }
        ...
        try {
            ...
            this.warn(`error in doDeviceQuery for '${this.internalDevLabel(deviceId)}': ${e && e.message ? e.message : e}`);
        }
    ...
    async addDevToGroup(devId, groupId, epid) {
        try {
            if (this.debugActive) this.debug(`called addDevToGroup with '${this.internalDevLabel(devId)}', ${groupId}, ${epid}`);
            const entity = await this.resolveEntity(devId);
            ...
            if (epid != undefined) {
                for (const ep of entity.endpoints) {
                    if (this.debugActive) this.debug(`checking ep ${ep.ID} of '${this.internalDevLabel(devId, entity.mapped?.model)}' (${epid})`);
                    if (ep.ID == epid) {
                        ...
                        else this.error(`cluster genGroups not supported for endpoint ${epid} of '${this.internalDevLabel(devId, entity.mapped?.model)}'`);
                    }
                }
            } else {
                if (entity.endpoint.inputClusters.includes(4)) {
                    this.info(`adding endpoint ${entity.endpoint.ID} of '${this.internalDevLabel(devId, entity.mapped?.model)}' to group`);
            ...
            this.sendError(error);
            this.error(`Exception when trying to Add '${this.internalDevLabel(devId)}'  to group ${groupId}`, error);
    ...
    async removeDevFromGroup(devId, groupId, epid) {
        if (this.debugActive) this.debug(`removeDevFromGroup with '${this.internalDevLabel(devId)}', ${groupId}, ${epid}`);
        ...
        try {
                ...
                if (epid != undefined) {
                    for (const ep of entity.endpoints) {
                        ...
                        if (ep.ID == epid && (ep.inputClusters.includes(4) || ep.outputClusters.includes(4))) {
                            ...
                            this.info(`removing endpoint ${ep.ID} of '${this.internalDevLabel(devId, entity.mapped?.model)}' from group ${groupId}`);
                        }
                    }
                } else {
                    await entity.endpoint.removeFromGroup(group);
                    this.info(`removing endpoint ${entity.endpoint.ID} of '${this.internalDevLabel(devId, entity.mapped?.model)}' from group ${groupId}`);
                }
            }
            else {
                const msg = `Unable to resolve group ${groupId} when removing '${this.internalDevLabel(devId, entity.mapped?.model)}' (ep ${epid ? epid : (entity ? entity.endpoint.ID : '')}) from group`
                this.error(msg);
                return {error: msg};
            }
            ...
            this.sendError(error);
            const msg = `Exception when trying remove '${this.internalDevLabel(devId, entity?.mapped?.model)}' (ep ${epid ? epid : (entity ? entity.endpoint.ID : '')}) from group ${groupId}`
    ...
    async removeDevFromAllGroups(devId) {
        try {
            ...
            this.sendError(error);
            this.error(`Exception when trying remove '${this.internalDevLabel(devId)}' from all groups`, error);
