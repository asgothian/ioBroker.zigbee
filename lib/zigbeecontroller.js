...
class ZigbeeController extends EventEmitter {
    ...
    */
    ...
    // try to read via converter
    //
    async readMultipleStates(deviceId, model, stateModel, stateList, _options, debugID, has_elevated_debug) {
        ...
        for (const stateDesc of stateModel.states) {
            if (!stateList.includes(stateDesc.id)) continue;
            const key = stateDesc.setattr || stateDesc.prop || stateDesc.id;

            if (stateDesc.isOption || stateDesc.isInternalState) continue;
            ...
            for (const candidate of source) {
                ...
                if (!converter && !candidate.hasOwnProperty['key]']) {
                    converter = candidate;
                }
                if (candidate.key.includes(key))
                    converter = candidate;
            }

            if (!converter) continue;

            this.warn(`reading ${key} for ep ${stateDesc.epname} of ${deviceId}`)

            await converter.convertGet(this.enpdointFromEpName(entity, stateDesc.epname) ,key, meta);
        }
    ...
    async publishFromState(deviceId, model, stateModel, stateList, options, debugID, has_elevated_debug) {
        ...
        const has_device_query = stateNames.indexOf('device_query') > -1;
        if (has_device_query && stateNames.length < 2) {
            await this.readMultipleStates(deviceId, model, stateModel, stateModel.states.map((obj) => obj?.id).filter((n) => n.indexOf('.') == -1), options, debugID, has_elevated_debug)
            return;
        }
        ...
        if (has_elevated_debug)
        {
            //const stateNames = stateList.map((state) => state.stateDesc.id).filter((s));
                            ...
                            for (const readKey of readKeys) {
                                if (readKey.converter) {
                                    ...
                                    setTimeout(async () => {
                                        try {
                                            const e = entity;
                                            await c.convertGet(target, key, {device:entity.device, publish:function(data) {
                                                this.warn(`publish called with ${JSON.stringify(data)} for ${utils.entityData(e)}`);
                                            }}) }
                                        catch (error) {
    ...
    recursiveCollectStates(exposes) {
        ...
        for (const expose of exposes) {
            if (expose.features) rv.push(...this.recursiveCollectStates(expose.features));
            const candidate = expose.property ?? expose.name;
    ...
    async doDeviceQuery(deviceId, debugID, elevated) {
        ...
        for (const target of targets) {
            ...
            try {
                await converter.convertGet(this.enpdointFromEpName(entity, target.epName) ,target.key, meta);
                read++;
                if (elevated) {
                    const message = `reading ${target.key} from Device ${this.devLabel(entity.device.ieeeAddr, mappedModel.model)}} ${target.epName ? 'Endpoint '+target.epName : ''}`;
                    this.emit('device_debug', { ID:debugID, data: { flag: `qs${attempt}` , IO:false }, message:message});
                }
            }
            catch (_e) {
                if (elevated) {
                    const message = `failed to read ${target.key} from Device ${this.devLabel(entity.device.ieeeAddr, mappedModel.model)}} ${target.epName ? 'Endpoint '+target.epName : ''}`;
                    this.emit('device_debug', { ID:debugID, data: { flag: `qe${attempt}` , IO:false }, message:message});
                }
                this.warn(`exception ${JSON.stringify(_e)}`);
                // intentionally empty
            }
        }
        ...
        if (elevated) {
            ...
            this.emit('device_debug', { ID:debugID, data: { flag: 'qe' , IO:false , payload: `queried ${read} of ${targets.length} possible states` }, message:message});
        }
        else
    ...
    async deviceQuery(deviceId, debugID, elevated, callback) {
        this.debug(`device query for ${deviceId} : active is ${JSON.stringify(this.deviceQueryActive)}`);
        ...
        try {
            ...
            this.warn(`error ${e?.message ?? 'no message'} in doDeviceQuery`)
        }
