    ...
    */
    ...
    async handleDeviceInterview(message) {
        ...
        */
        try {
            const entity = await this.resolveEntity(message.device || message.ieeeAddr);
            const friendlyName = this.devLabel(entity.device.ieeeAddr, entity.mapped?.model);
    ...
    async doDeviceQuery(deviceId, debugID, elevated, keyList) {
        ...
        if (mappedModel) {
            ...
            if (elevated) {
                ...
                if (this.debugActive) this.debug(`Device query for '${this.devLabel(entity.device.ieeeAddr, mappedModel.model)}' started`);
                else this.info(`Device query for '${this.devLabel(entity.device.ieeeAddr, mappedModel.model)}' started`);
                        ...
                        for (const k of converter.key)
                            if (keyList === undefined || keyList.includes(k))
                                try {
                                    ...
                                    if (elevated) {
                                        const message = `read for state ${k} of '${converter.key.join(',')}' of '${this.devLabel(entity.device.ieeeAddr, mappedModel.model)}/${source.ID}' after device query`;
                                        this.warn(`ELEVATED O02.1 ${message}`);
                                        this.emit('device_debug', { ID:debugID, data: { flag: '03', IO:false }, message:message });
                                    }
                                    else
                                        this.debug(`read for state${converter.key.length ? '' : 's'} '${converter.key.join(',')}' of '${this.devLabel(entity.device.ieeeAddr, mappedModel.model)}/${source.ID}' after device query`);
                                } catch (error) {
            ...
            if (elevated) {
                const message = `ELEVATED O07: Device query for '${this.devLabel(entity.device.ieeeAddr, mappedModel.model)}}' complete`;
                this.emit('device_debug', { ID:debugID, data: { flag: 'qe' , IO:false , payload }, message:message});
            }
            else
                this.info(`Device query for '${this.devLabel(entity.device.ieeeAddr, mappedModel.model)}' complete`);
        }
    }
