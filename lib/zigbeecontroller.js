...
class ZigbeeController extends EventEmitter {
    ...
    */
    ...
    async zbDeviceCommand(command, devId) {
        let result = {error: 'zbDeviceCommand: illegal command ' + command};
        switch (command) {
            case 'configure':
            case 'reconfigure':
                result = await this.reconfigure(devId);
                break;
            case 'interview':
                result = await this.interview(devId);
                break;
        }
    }
    ...
    async interview(devId)
    {
        ...
        if (entity) try {
            ...
            await entity.device.interview();
            this.info(`interview for ${entity.device.ieeeAddr} ${entity.device.modelID} complete`);
        } catch (error) {
            this.error(`Interview failed ${entity.device.ieeeAddr} ${entity.device.modelID}, (${error.message})`);
