            ...
            stateList.forEach(async changedState => {
                ...
                if (options?.meta_state) {
                    ...
                    for (const state of parts) {
                        ...
                        if (!(meta.state && meta.state.hasOwnProperty(tstate))) {
                            const s = await this.adapter.getState(`${utils.zbIdorIeeetoAdId(this.adapter, deviceId, false)}.${tstate}`);
                            if (stateDesc.setter) {
                                meta.state[tstate] = stateDesc.setter(s.val)
                            } else meta.state[tstate] = s.val;
                        }
                    }
                }
