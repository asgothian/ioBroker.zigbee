    ...
    */
    ...
    async start() {
        ...
        try {
            ...
            for (const device of deviceIterator) {
                ...
                if (!entity) {
                    this.debug('failed to resolve Entity for ' + "'" + this.deviceLabel(device.ieeeAddr) + "'");
                    continue;
                }
                ...
                const msg = ("'" + this.deviceLabel(entity.device.ieeeAddr, entity.mapped?.model) + "'" +
        ...
        for (const device of iterator) {
            for (const ep of device.endpoints) {
                ...
                if (ep_binds && ep_binds.length) {
                    const entity = resolvedEntities[device.ieeeAddr] ?? await this.resolveEntity(device) ?? { };
                    resolvedEntities[device.ieeeAddr] = entity;
                    const bindObj = {
                        address: device.ieeeAddr,
                        endpoint: ep.ID,
                        binds: {
                        },
                        name: this.deviceLabel(device.ieeeAddr, entity?.mapped?.model ?? device.modelID)
                    }
                    ...
                    for (const bind of ep_binds) {
                        ...
                        if (!bindObj.binds.hasOwnProperty(ckey)) {
                            const entity = resolvedEntities[device.ieeeAddr] ?? this.resolveEntity(device.ieeeAddr) ?? { };
                            resolvedEntities[bind.target.deviceIeeeAddress] = entity;
                            bindObj.binds[ckey] = {
                                ...
                                name: this.deviceLabel(device.ieeeAddr, entity?.mapped?.model ?? entity?.device?.modelID)
                            }
                        }
    ...
    async zbDeviceCommand(command, devId, statedesc) {
        ...
        try {
            ...
            this.error(`the command ${command} failed for '${this.deviceLabel(devId)}': ${error?.message}`);
        }
        return false;
    }
    ...
    async reconfigure(devId) {
        ...
        this.debug(`Reconfigure '${this.deviceLabel(devId)}'`);
    ...
    async interview(devId)
    {
        const entity = await this.resolveEntity(devId);
        if (entity) try {
            this.info(`manual Interview for '${this.deviceLabel(entity.device.ieeeAddr, entity.mapped?.model)}'`);
            await entity.device.interview(true);
            this.info(`interview for '${this.deviceLabel(entity.device.ieeeAddr, entity.mapped?.model)}' complete`);
        } catch (error) {
    ...
    async remove(deviceID, force) {
        try {
            ...
            const msg = `Failed to remove ${deviceID ? 'device ' + "'" + this.deviceLabel(deviceID) + "'" : 'unspecified device'}: ${message}`;
        ...
        if (this.localConfig.optionExists('ignore_device_delete')) {
            ...
            if (entity) {
                if (this.localConfig.getDeviceOption(entity.device.ieeeAddr, entity.mapped?.model, 'ignore_device_delete', true)) {
                    this.warn(`Ignoring device_leave message from '${this.deviceLabel(message.ieeeAddr, entity.mapped?.model)}' (${entity.device.modelID})`);
                    return;
                }
            }
        }
        ...
        try {
            ...
            for (const device of devIter)
            {
                ...
                if (entity.name === 'unresolved device' && this.debugActive) {
                    this.debug('resolve Entity failed for ' + "'" + this.deviceLabel(device.ieeeAddr) + "'");
                }
    ...
    async publish(deviceID, cid, cmd, zclData, cfg, ep, type, callback, zclSeqNum) {
        ...
        if (this.debugActive) this.debug(`Zigbee publish to '${this.deviceLabel(deviceID, entity.mapped?.model)}', ${cid} - cmd ${cmd} - payload ${JSON.stringify(zclData)} - cfg ${JSON.stringify(cfg)} - endpoint ${ep}`);
    ...
    async publishFromState(deviceId, model, stateModel, stateList, options, debugID, has_elevated_debug) {
        ...
        if (has_elevated_debug)
        {
            ...
            const message = `Publishing to '${this.deviceLabel(deviceId, model)}' of model ${model} with ${stateNames.join(', ')}`;
            this.emit('device_debug', { ID:debugID, data: { ID: deviceId, flag: '03', IO:false }, message: message});
        }
        else
            if (this.debugActive) this.debug(`main publishFromState : '${this.deviceLabel(deviceId, model)}' ${model} ${safeJsonStringify(stateList)}`);
        if (model === 'group') {
            isGroup = true;
        ...
        try {
            ...
            if (this.debugActive) this.debug(`entity: '${this.deviceLabel(deviceId, model)}' ${model} ${safeJsonStringify(utils.entityData(entity,{entity:true, device:true, model: true}))}))}`);
            const mappedModel = entity ? entity.mapped : undefined;

            if (!mappedModel) {
                if (this.debugActive) this.debug(`No mapped model for ${model}`);
                if (has_elevated_debug) {
                    const message=`No mapped model '${this.deviceLabel(deviceId, model)}' (model ${model})`;
            ...
            for (const changedState of stateList) {
                ...
                if (stateDesc.isOption) {
                    if (has_elevated_debug) {
                        const message = `No converter needed on option state for '${this.deviceLabel(deviceId, model)}' of type ${model}`;
                        this.emit('device_debug', { ID:debugID, data: { flag: `SUCCESS` , IO:false }, message:message});
                    }
                    else
                        if (this.debugActive) this.debug(`No converter needed on option state for '${this.deviceLabel(deviceId, model)}' of type ${model}`);
                ...
                for (const c of mappedModel.toZigbee) {
                    ...
                    if (!c.hasOwnProperty('key')) {
                        if (converter === undefined)
                        {
                            ...
                            converter = c;
                            if (has_elevated_debug) {
                                const message = `Setting converter to keyless converter for '${this.deviceLabel(deviceId, model)}' of type ${model}`;
                                this.emit('device_debug', { ID:debugID, data: { flag: `s4.${msg_counter}` , IO:false }, message:message});
                            }
                            else
                                if (this.debugActive) this.debug(`Setting converter to keyless converter for '${this.deviceLabel(deviceId, model)}' of type ${model}`);
                            msg_counter++;
                        }
                        else
                        {
                            if (has_elevated_debug)
                            {
                                const message = `ignoring keyless converter for '${this.deviceLabel(deviceId, model)}' of type ${model}`;
                                this.emit('device_debug', { ID:debugID, data: { flag: `i4.${msg_counter}` , IO:false} , message:message});
                            }
                            else
                                if (this.debugActive) this.debug(`ignoring keyless converter for '${this.deviceLabel(deviceId, model)}' of type ${model}`);
                            msg_counter++;
                        }
                        continue;
                    }
                ...
                const message = `convert ${key} with value ${safeJsonStringify(preparedValue)} and options ${safeJsonStringify(preparedOptions)} for device '${this.deviceLabel(deviceId, model)}' ${stateDesc.epname ? `with Endpoint ${epName}` : 'without Endpoint'}`;
                if (has_elevated_debug) {
                ...
                do
                {
                    try {
                        ...
                        */
                        ...
                        const message = `convert result ${safeJsonStringify(result)} for device '${this.deviceLabel(deviceId, model)}'`;
                        if (isGroup)
                        ...
                        if (result !== undefined) {
                            ...
                            if (has_elevated_debug) {
                                ...
                                const message = `Convert does not return a result result for ${key} with ${stringvalue} on device '${this.deviceLabel(deviceId, model)}'.`;
                                ...
                                if (readKey.converter) {
                                    ...
                                    setTimeout(async () => {
                                        try {
                                            ...
                                            this.warn(`error reading ${key} from '${this.deviceLabel(deviceId, model)}' : ${error?.message}`);
                                        }
                                    }, tt ? tt * 1000+250 : 1000);
                                }
                            }
                        ...
                        if (error.code === 25) {
                            if (retry > 0) {
                                this.warn(`Error ${error.code} on send command to '${this.deviceLabel(deviceId, model)}'. (${retry} tries left.), Error: ${error.message}`);
                                retry--;
                                ...
                                this.adapter.filterError(`Error ${error.code} on send command to '${this.deviceLabel(deviceId, model)}'.` +
                                ` Error: ${error.message}`, `Send command to ${deviceId} failed with`, error, `send:${String(deviceId).replace('0x', '')}`);
                                if (has_elevated_debug) {
                                    const message = `caught error ${error?.message ?? 'no reason given'} when setting value for device '${this.deviceLabel(deviceId, model)}'.`;
                            ...
                            ` Error: ${error.message}`, `Send command to ${deviceId} failed with`, error, `send:${String(deviceId).replace('0x', '')}`);
                            if (has_elevated_debug) {
                                const message = `caught error ${error?.message ?? 'no reason given'} when setting value for device '${this.deviceLabel(deviceId, model)}' (${error?.stack ?? ''}).`;
                                this.emit('device_debug', { ID:debugID, data: { error: `EXSET${error.code == 25 ? retry : ''}` , IO:false },message:message});
                            }
                        }
                    }
            ...
            const message = `No entity for '${this.deviceLabel(deviceId, model)}' : ${err && err.message ? err.message : 'no error message'}`;
            if (has_elevated_debug) {
        ...
        if (payloadObj.hasOwnProperty('device') && payloadObj.hasOwnProperty('payload')) {
            try {
                ...
                try {
                    ...
                    this.error(`Error ${error.code} on send command to '${this.deviceLabel(payloadObj.device, mappedModel.model)}'.` + ` Error: ${error.stack} ` + `Send command to '${this.deviceLabel(payloadObj.device, mappedModel.model)}' failed with ` + error);
                    this.adapter.filterError(`Error ${error.code} on send command to '${this.deviceLabel(payloadObj.device, mappedModel.model)}'.` + ` Error: ${error.stack}`, `Send command to '${this.deviceLabel(payloadObj.device, mappedModel.model)}' failed with`, error);
    ...
    async doDeviceQuery(deviceId, debugID, elevated) {
        ...
        if (this.debugActive) this.debug(`doDeviceQuery: resolveEntity for entity: '${this.deviceLabel(deviceId)}' is ${safeJsonStringify(utils.entityData(entity, {}))}`);

        if (elevated) {
            const message  = `Device query for '${this.deviceLabel(entity.device.ieeeAddr, mappedModel.model)}' triggered`;
            ...
            this.info(`Device query for '${this.deviceLabel(entity.device.ieeeAddr, mappedModel.model)}' started`);
        }
        const meta = await utils.getMeta(this.adapter, `${utils.zbIdorIeeetoAdId(this.adapter, deviceId, false)}`);
        ...
        for (const target of targets) {
            ...
            try {
                ...
                read++;
                if (elevated) {
                    const message = `reading ${target.key} from Device ${this.deviceLabel(entity.device.ieeeAddr, mappedModel.model)}} ${target.epName ? 'Endpoint '+target.epName : ''} in ${(Date.now()-start)} ms`;
                    this.emit('device_debug', { ID:debugID, data: { flag: `qs${attempt}` , IO:false }, message:message});
                }
            }
            catch (e) {
                if (elevated) {
                    const message = `failed to read ${target.key} from Device ${this.deviceLabel(entity.device.ieeeAddr, mappedModel.model)}} ${target.epName ? 'Endpoint '+target.epName : ''} after ${(Date.now()-start)} ms`;
        ...
        if (elevated) {
            const message = `Device query for '${this.deviceLabel(entity.device.ieeeAddr, mappedModel.model)}}' complete`;
            ...
            this.info(`Device query for '${this.deviceLabel(entity.device.ieeeAddr, mappedModel.model)}' complete, read ${read} of ${targets.length} possible states`);

    }

    async deviceQuery(deviceId, debugID, elevated, callback) {
        if (this.deviceQueryActive.includes (deviceId)) {
            this.info(`Device query for '${this.deviceLabel(deviceId)}' is still active.`);
            return;
        }
        ...
        try {
            ...
            this.warn(`error in doDeviceQuery for '${this.deviceLabel(deviceId)}': ${e && e.message ? e.message : e}`);
        }
    ...
    async addDevToGroup(devId, groupId, epid) {
        try {
            if (this.debugActive) this.debug(`called addDevToGroup with '${this.deviceLabel(devId)}', ${groupId}, ${epid}`);
            const entity = await this.resolveEntity(devId);
            ...
            if (epid != undefined) {
                for (const ep of entity.endpoints) {
                    if (this.debugActive) this.debug(`checking ep ${ep.ID} of '${this.deviceLabel(devId, entity.mapped?.model)}' (${epid})`);
                    if (ep.ID == epid) {
                        ...
                        else this.error(`cluster genGroups not supported for endpoint ${epid} of '${this.deviceLabel(devId, entity.mapped?.model)}'`);
                    }
                }
            } else {
                if (entity.endpoint.inputClusters.includes(4)) {
                    this.info(`adding endpoint ${entity.endpoint.ID} of '${this.deviceLabel(devId, entity.mapped?.model)}' to group`);
            ...
            this.sendError(error);
            this.error(`Exception when trying to Add '${this.deviceLabel(devId)}'  to group ${groupId}`, error);
    ...
    async removeDevFromGroup(devId, groupId, epid) {
        if (this.debugActive) this.debug(`removeDevFromGroup with '${this.deviceLabel(devId)}', ${groupId}, ${epid}`);
        ...
        try {
                ...
                if (epid != undefined) {
                    for (const ep of entity.endpoints) {
                        ...
                        if (ep.ID == epid && (ep.inputClusters.includes(4) || ep.outputClusters.includes(4))) {
                            ...
                            this.info(`removing endpoint ${ep.ID} of '${this.deviceLabel(devId, entity.mapped?.model)}' from group ${groupId}`);
                        }
                    }
                } else {
                    await entity.endpoint.removeFromGroup(group);
                    this.info(`removing endpoint ${entity.endpoint.ID} of '${this.deviceLabel(devId, entity.mapped?.model)}' from group ${groupId}`);
                }
            }
            else {
                const msg = `Unable to resolve group ${groupId} when removing '${this.deviceLabel(devId, entity.mapped?.model)}' (ep ${epid ? epid : (entity ? entity.endpoint.ID : '')}) from group`
                this.error(msg);
                return {error: msg};
            }
            ...
            this.sendError(error);
            const msg = `Exception when trying remove '${this.deviceLabel(devId, entity?.mapped?.model)}' (ep ${epid ? epid : (entity ? entity.endpoint.ID : '')}) from group ${groupId}`
    ...
    async removeDevFromAllGroups(devId) {
        try {
            ...
            this.sendError(error);
            this.error(`Exception when trying remove '${this.deviceLabel(devId)}' from all groups`, error);
